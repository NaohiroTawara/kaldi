...
void GradientReversalComponent::Init(int32 dim, BaseFloat lambda) {
  parameter_ = lambda;
  dim_ = dim;
}
...
void GradientReversalComponent::InitFromConfig(ConfigLine *cfl) {
  BaseFloat lambda = 0;
  int32 dim = 0;
  bool ok = cfl->GetValue("lambda", &lambda) &&
    cfl->GetValue("input-dim", &dim);
...
void GradientReversalComponent::Backprop(const std::string &debug_info,
  ...
  in_deriv->Scale(parameter_);
}

void GradientReversalComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<GradientReversalComponent>", "<Parameter>");
