...
void SetParameter(BaseFloat parameter,
		  Nnet *nnet) {
  for (int32 c = 0; c < nnet->NumComponents(); c++) {
    Component *comp = nnet->GetComponent(c);
    if (comp->Properties() & kChangeableParameter) {
      // For now all updatable components inherit from class UpdatableComponent.
      // If that changes in future, we will change this code.
      ChangeableParameterComponent *uc = dynamic_cast<ChangeableParameterComponent*>(comp);
      if (uc == NULL)
        KALDI_ERR << "Updatable component does not inherit from class "
            "UpdatableComponent; change this code.";
      uc->SetParameter(parameter);
    }
  }
}
