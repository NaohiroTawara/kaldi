...
///   nnet-general-component.h there are components that don't fit this pattern.
 ...
 class GradientReversalComponent: public ChangeableParameterComponent {
 public:
  void Init(int32 dim, BaseFloat lambda);
  explicit GradientReversalComponent(int32 dim, BaseFloat lambda) {
    Init(dim, lambda);
  }
  virtual int32 Properties() const {
    return kSimpleComponent|kPropagateInPlace|kChangeableParameter;
  }
  ...
  virtual Component* Copy() const { return new GradientReversalComponent(dim_, parameter_); }
