...
static bool ProcessFile(const GeneralMatrix &feats,
  ...
  for (size_t c = 0; c < chunks.size(); c++) {
    ...
    Posterior phones(num_frames_subsampled);
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
      ...
      "\n"
      "Usage:  nnet3-get-egs-dense-targets --num-targets=<n> --num-phns=<n> [options] "
      "<features-rspecifier> <targets-rspecifier> <pdf-post-rspecifier> <egs-out>\n"
      "\n"
	...
	const Posterior &pdf_post = pdf_post_reader.Value(key);
        if (!ProcessFile(feats, online_ivector_feats, online_ivector_period,
                         target_matrix, pdf_post, num_phns, key, compress, num_targets, 
