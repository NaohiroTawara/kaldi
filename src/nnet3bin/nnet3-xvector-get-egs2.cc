...
static void WriteExamples(const MatrixBase<BaseFloat> &feats,
			  ...
			  bool compress, int32 num_pdfs, 
			  const Posterior &pdf_post, int32 num_phns,
			  int32 *num_egs_written,
			  std::vector<NnetExampleWriter *> *example_writers) {
  for (std::vector<ChunkInfo *>::const_iterator it = chunks.begin(); it != chunks.end(); ++it) {
    ChunkInfo *chunk = *it;
    NnetExample eg;
    ...
    if (num_rows < chunk->num_frames) {
      ...
      Posterior phones(chunk->num_frames);
      for (int32 i = 0; i < chunk->num_frames; i++){
				int32 t = i + chunk->start_frame + shift;
				phones[i] = pdf_post[t];
      }
      eg.io.push_back(NnetIo("output_pdfs", num_phns, 0, phones));
      if (compress)
        eg.Compress();
      ...
      if (chunk->output_archive_id >= example_writers->size())
        KALDI_ERR << "Requested output index exceeds number of specified "
                  << "output files.";
      (*example_writers)[chunk->output_archive_id]->Write(chunk->name, eg);
      (*num_egs_written) += 1;
    }
  }
}
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "<features-rspecifier> <pdf-post-rspecifier> <egs-0-out> <egs-1-out> ... <egs-N-1-out>\n"
        "\n"
        "For example:\n"
        "nnet3-xvector-get-egs ranges.1 \"$feats\"\\\n"
	"\"ark:gunzip -c exp/nnet/ali.1.gz | ali-to-pdf exp/nnet/1.nnet ark:- ark:- | ali-to-post ark:- ark:- |\" \\\n"
	"  ark:egs_temp.1.ark ark:egs_temp.2.ark ark:egs_temp.3.ark\n";
    ...
    po.Register("num-pdfs", &num_pdfs, "Number of speakers in the training "
                "list.");
    po.Register("num-phns", &num_phns, "Number of phones in the training "
                "list.");
